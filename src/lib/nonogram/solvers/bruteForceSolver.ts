 ...
 */
let solveGap = (gap: number[], hints: number[]) => {
  ...
  if (cacheLimits[0] <= hints.length && hints.length <= cacheLimits[1]) {
    let candidate = (solveGap as any).cache[JSON.stringify([gap, hints])];
    if (candidate) {
      return candidate;
    }
  }
...
let solveGapWithHintList = (
  ...
  if (debugMode) {
    if (gap.some((x, i) => x !== result[i])) {
      console.log(
        `Gap solved: [${gap}], ${JSON.stringify(hintList)} -> ${result}`
      );
    } else {
...
let solve = (line: Cell[], hints: number[], debugMode = false) => {
  ...
  util.assert(
    ...
    `Contradiction in line ${line} | ${hints}`
  );
  ...
  let changed = new Set();
  gaps.forEach((gap, i) => {
    let gapResult = solveGapWithHintList(
